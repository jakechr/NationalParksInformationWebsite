document.getElementById("weatherSubmit").addEventListener("click", function(event) {
  ...
  if (value === "")
    return;
  const url = "http://api.openweathermap.org/data/2.5/weather?q=" + value + ",US&units=imperial" + "&APPID=60347f560a3b028d1ea0ceda27cb0ea4";
  fetch(url)
    .then(function(response) {
      ...
      if (json.cod == '404') {
        ...
        if (json.weather[0].main.toLowerCase().includes("lightning")) {
          backgroundImage = 'url(images/lightningbg.jpg)';
        } else if (json.weather[0].main.toLowerCase().includes("rain")) {
          backgroundImage = 'url(images/rainybg.jpg)';
        } else if (json.weather[0].main.toLowerCase().includes("cloud")) {
          backgroundImage = 'url(images/cloudybg.jpg)';
        } else if (json.weather[0].main.toLowerCase().includes("sun") || results.toLowerCase().includes("clear sky")) {
          backgroundImage = 'url(images/sunnybg.jpg)';
        } else if (json.weather[0].main.toLowerCase().includes("snow")) {
          backgroundImage = 'url(images/snowybg.jpg)';
        } else if (json.weather[0].main.toLowerCase().includes("clear")) {
          backgroundImage = 'url(images/clearskybg.jpg)';
        }
      }
